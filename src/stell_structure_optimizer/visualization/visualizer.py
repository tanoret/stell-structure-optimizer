...
class Visualizer:
    ...
    def __init__(self, analyzer: FrameAnalyzer):
        """
        Initializes the Visualizer with a solved FrameAnalyzer instance.
        """
    ...
    def plot(self, result_type: str = 'moment_z', title: str = None, line_width: int = 5):
        """
        Generates a 3D plot of the UNDEFORMED structure with beams colored by a result.
        """
        fig = plt.figure(figsize=(14, 10))
        ax = fig.add_subplot(111, projection='3d')

        is_abs = 'abs' in result_type or 'mag' in result_type or result_type == 'stress'
        ...
        ax.set_xlabel("Global X (m)"); ax.set_ylabel("Global Z (m)"); ax.set_zlabel("Global Y (m) - Elevation")
        ax.set_title(title, fontsize=16)

        # --- THIS IS THE FIX ---
        # The correct axis limit and aspect ratio logic is now included.
        self._set_plot_aspect_ratio(ax)

        plt.show()

    def plot_deformed_shape(self, result_type: str = 'displacement_magnitude', amplification_factor: float = 50.0, title: str = None, line_width: int = 4):
        """
        Generates a 3D plot of the DEFORMED shape, colored by a result quantity.
        """
        ...
        for i, beam in enumerate(self.beams):
            ...
            val1 = np.mean(nodal_values[n1_idx])
            val2 = np.mean(nodal_values[n2_idx])
    ...
    def _get_result_data(self, result_type, for_beam_ends=False):
        ...
        if 'disp' in result_type:
            ...
            for r in self.results:
                ...
                v1,v2=0,0
                if result_type=='axial_force': v1,v2,label = r['node1']['N'], r['node2']['N'], "Axial Force N (Tension +)"
                elif result_type=='shear_y': v1,v2,label = r['node1']['Vy'], r['node2']['Vy'], "Local Shear Vy (N)"
                elif result_type=='shear_z': v1,v2,label = r['node1']['Vz'], r['node2']['Vz'], "Local Shear Vz (N)"
                elif result_type=='stress': v1,v2,label,cmap = max(abs(v) for v in r['node1']['stresses'].values()), max(abs(v) for v in r['node2']['stresses'].values()), "Max Corner Stress (Pa)",'viridis'
                elif 'mag' in result_type:
                    label,cmap = "Magnitude", 'viridis'
                    if 'shear' in result_type: v1,v2,label = np.sqrt(r['node1']['Vy']**2 + r['node1']['Vz']**2), np.sqrt(r['node2']['Vy']**2 + r['node2']['Vz']**2), "Shear Force Magnitude (N)"
                    if 'moment' in result_type: v1,v2,label = np.sqrt(r['node1']['My']**2 + r['node1']['Mz']**2), np.sqrt(r['node2']['My']**2 + r['node2']['Mz']**2), "Bending Moment Magnitude (Nm)"
                elif 'moment' in result_type:
                    key,cmap = ('My' if 'y' in result_type else 'Mz'),'viridis'
                    v1,v2,label = abs(r['node1'][key]), abs(r['node2'][key]), f"|Local Moment {key}| (Nm)"
                else: raise ValueError(f"Unknown result_type: '{result_type}'")
                nodal_values[b.node1_idx].append(v1); nodal_values[b.node2_idx].append(v2)
                if for_beam_ends: beam_end_vals.append((v1, v2))
    ...
    def _set_plot_aspect_ratio(self, ax):
        ...
        # Find the center and range of the data
        x_c, y_c, z_c = np.mean(all_x), np.mean(all_y), np.mean(all_z)
        x_r, y_r, z_r = np.ptp(all_x), np.ptp(all_y), np.ptp(all_z)

        # --- THIS IS THE FIX ---
        # Ensure that even if the data is 1D or 2D, the plot box has 3D volume.
        # We find the maximum range and use it to define the plot box,
        # preventing any dimension from having a zero size.
        ...
        # This call will now work correctly since no axis has a zero range.
        ax.set_box_aspect((1, 1, 1)) # Force equal aspect ratio
